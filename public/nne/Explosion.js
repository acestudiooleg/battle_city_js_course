 ...
 * - wall: малий вибух при зіткненні з деревянними стінами (1 HP)
 * - armor: середній вибух при зіткненні з цегляними стінами (3 HP) або бронею танка
 * - tank: великий вибух при знищенні бетонних стін (10 HP) або танка
 */
...
export class Explosion {
    constructor(x, y, type = 'wall', size = 20, logger = null) {
        ...
        if (this.logger) {
            this.logger.gameEvent(`💥 Вибух типу ${type} створений`, { x, y, size: this.size, type });
        }
    ...
    getDurationByType(type) {
        switch (type) {
            case 'wall':
                return 600; // 0.6 секунди для дерева
            case 'armor':
                return 1000; // 1 секунда для цегли
            case 'tank':
                return 1800; // 1.8 секунди для бетону
            default:
                return 800;
        }
    }
    ...
    getColorsByType() {
        switch (this.type) {
            ...
            case 'tank':
                return ['#e74c3c', '#c0392b', '#8e44ad', '#2c3e50', '#f39c12', '#f1c40f']; // Різноманітні для бетону
            default:
